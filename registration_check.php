...
if($addres === '')
{
  print '<p class="check-p">住所が入力されてません</p><br>';
}
else
{
...
{
  print '<p class="check-p">電話番号を正確に入力してください</p><br>';
}
else
{
...
if($pass1 === '')
{
  print '<p class="check-p">パスワードを入力してください</p><br>';
}
if($pass1 !== $pass2 )
{
  print '<p class="check-p">パスワードが一致していません</p><br>';
}
...
if($name === '' || preg_match('/\A[\w\-\.]+\@[\w\-\.]+\.([a-z]+)\z/',$mail) === 0 || preg_match('/\A[0-9]+\z/',$post1) === 0 || preg_match('/\A[0-9]+\z/',$post2) === 0 || $addres === '' || preg_match('/\A\d{2,5}-?\d{2,5}-?\d{4,5}\z/',$tel) === 0 || $pass1 !== $pass2)
{
  print '<form>';
...
{
print '<form action="registration_insert.php" method="post">';
