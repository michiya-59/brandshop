...
// product_edit.check8.phpから送られてきた、変更するため情報をPOSTで受け取る
try
{
...
$sql = 'UPDATE mst_product SET name=?,price=?,gazou=?,gazou2=?,gazou3=?,gazou4=?,gazou5=?,gazou6=?,gazou7=?,gazou8=?,size=?,condetion=?,detail=?,stock=?,category=?,brand=? WHERE code=?';
...
if($product_gazou_name_old7 !== $product_gazou_name7)
{
  if($product_gazou_name_old7 !== '')
  {
    unlink('./gazou/'.$product_gazou_name_old7);
  }
}
if($product_gazou_name_old8 !== $product_gazou_name8)
{
  if($product_gazou_name_old8 !== '')
  {
    unlink('./gazou/'.$product_gazou_name_old8);
  }
}
// 終わり
