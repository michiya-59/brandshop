...
if($product_name === '' || preg_match('/\A[0-9]+\z/',$product_price) === 0 || $product_size === '' || $product_detail === '')  //入力間違えが無い場合に以下の処理をさせる
{
  print '<form>';
...
else
{
  ...
  switch($product_gazou)
  {
    ...
    case '4':
      ...
      print '<input type="file" name="gazou1" style="width:400px"><br>';//ここからが4枚の画像をtype="file"で選択する処理
      print '<input type="file" name="gazou2" style="width:400px"><br>';
      print '<input type="file" name="gazou3" style="width:400px"><br>';
      print '<input type="file" name="gazou4" style="width:400px"><br><br>'; //ここまでが4枚の画像をtype="file"で選択する処理
      print '<input type="hidden" name="name" value="'.$product_name.'">';  //ここからPOSTで受け取った情報をhiddenで次の処理に向けて渡す処理
      ...
      print '<input type="hidden" name="brand" value="'.$product_brand.'">';//ここまでがPOSTで受け取った情報をhiddenで次の処理に向けて渡す処理
      print '<input type="button" onclick="history.back()" value="戻る">';
      print '<input type="submit" value="OK">';
      print '</form>';
      break;
  
      // 上記と同じで写真を5枚追加するときの処理等
    case '5':
